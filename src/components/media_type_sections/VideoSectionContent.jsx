        ...
        <>
            <div className={`${videoSecWidth} ${videoSecHeight} rounded-lg relative`}>
                {imageData.media_type === "video" ? (
                    <Link to={imageData.url} target='_blank'>
                ...
                <div className={`w-full flex flex-col items-center justify-center flex-grow`}>
                    {imageData.media_type === "video" ? (
                        <div className={`${buttonWidth}`}>
                            <Link to={imageData.url} target='_blank'><button className='btn rounded-3xl apod-button equal-button'>Watch Video</button></Link>
                        </div>
                    ) : (
                        <div className={`${buttonWidth}`}>
